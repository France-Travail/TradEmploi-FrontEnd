...
export class MessageWrapperComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.user === 'advisor') {
      ...
      this.flag = VOCABULARY_V2.find(item => item.isoCode === this.settingsService.advisor.language)
        .sentences.find(s => s.key === 'flag')
        .value.toLowerCase();
      ...
      this.flag = VOCABULARY_V2.find(item => item.isoCode === this.settingsService.guest.value.language)
        .sentences.find(s => s.key === 'flag')
        .value.toLowerCase();
    }
